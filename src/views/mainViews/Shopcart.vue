<template>
  <div v-if="isLogin">
    <van-list class="shopcart" v-model="loadOptions.isLoading" :finished="loadOptions.isFinished" :finished-text="loadOptions.finishedText" @load="loadData" :offset="50">
      ...
      <div v-else>
        <van-nav-bar @click-right="manage" title="购物车" :right-text="isManage ? '管理' : '完成'" />
        ...
        <!-- 提交订单 -->
        <div v-if="isManage">
          <van-submit-bar class="submit-bar" :price="total" :disabled="isNotHasCheck" button-text="提交订单" button-type="info" @submit="sumbitOrder">
            <van-checkbox v-model="allCheck" @click="allSelect">全选</van-checkbox>
          </van-submit-bar>
        </div>
        <div v-else>
          <van-submit-bar class="submit-bar submit-bar-delete" :disabled="isNotHasCheck" button-text="删除选择" button-type="info" @submit="removeMoreShopcart">
            <van-checkbox v-model="allCheck" @click="allSelect">全选</van-checkbox>
          </van-submit-bar>
        </div>
      </div>
